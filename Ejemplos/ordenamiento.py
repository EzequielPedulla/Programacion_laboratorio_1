...
lista_numeros = [0, 6, 5, 8, 1, 2, 10, 62]
...
resultado = selection_sort_v1(lista_numeros)
...
def particionar_lista(lista_original: list) -> list:
    ...
    # la lista derecha va a contener los numeros mayores al pivot
    lista_de = []

    # la lista derecha va a contener los numeros menores al pivot
    lista_iz = []

    if (len(lista) <= 1):  # si la lista es 0 o 1 elemento no tiene sentido ordenarla
        return lista_original
    else:
        pivot = lista[0]  # elegimos el primer elemento como pivot

        for elemento in lista[1:]:  # la lista recorre desde el elemento 1

            if elemento > pivot:
                lista_de.append(elemento)  # los mayores van a la lista derecha
            else:
                # los menores van a la lista izquierda
                lista_iz.append(elemento)

        return lista_iz, pivot, lista_de  # retornamos para que queden ordenados
